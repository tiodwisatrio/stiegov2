    ...
    <form action="{{ route('admin.highlights.store') }}" method="POST" class="space-y-6">
        ...
        <div x-data="{
            ...
            <div @click="open = !open" class="cursor-pointer">
                <div x-show="!selectedProduct" class="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 shadow-sm focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500">
                ...
                <div x-show="selectedProduct" class="mt-1 flex items-center gap-3 rounded-md border border-gray-300 bg-white px-3 py-2 shadow-sm hover:border-indigo-400">
                    ...
                    <button type="button" @click.stop="selectedProduct = null" class="text-gray-400 hover:text-gray-600">
        ...
        <div>
            ...
            <select name="highlight_type_id" id="highlight_type_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                ...
                @foreach($highlightTypes as $type)
                    <option value="{{ $type->id }}" {{ old('highlight_type_id') == $type->id ? 'selected' : '' }}>
                        {{ $type->name }}
                    </option>
                @endforeach
            </select>
